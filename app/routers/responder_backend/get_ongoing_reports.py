...
@router.get("/get_ongoing_reports")
def get_ongoing_reports(responder_id: int):
    conn = get_db_conn()
    cursor = conn.cursor()
    try:
        cursor.execute(
            ...
            WHERE responder_id = %s AND status IN ('Not Resolved', 'Pending')
            ORDER BY id DESC
            """,
            (responder_id,)
        )
        ...
        print("Get ongoing reports error:", e)
